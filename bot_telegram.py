...
# Función para manejar mensajes de texto y ejecutar el script adb_generator.py si el comando es "transfer <banco>"
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if message_text.startswith("transfer"):
        ...
        elif message_text.startswith("client"):
            await handle_client_command(update, context, message_text)
        elif message_text.startswith("save"):
            await handle_save_command(update, context, message_text)
        else:
            await update.message.reply_text("Comando no reconocido. Por favor, envía 'transfer <banco> <cuenta> <monto> <comentario>', 'status <banco>', 'client <banco> <cliente> <monto> <comentario>', o 'save <banco> <cuenta> <cedula> <cliente> <monto> <comentario>'.")
